...
class ktensor:
    ...
    def redistribute(self, mode):
        ...
        """
        #print 'mode',mode
        #print 'self.U[mode]',self.U[mode].shape
        #print 'self.lmbda[np.newaxis, :]',self.lmbda[np.newaxis, :].shape
        self.U[mode]=np.array(self.U[mode])
        self.lmbda[np.newaxis, :]=np.array(self.lmbda[np.newaxis, :])

...
The code is the python implementation of the @sptenmat folder in the MATLAB Tensor Toolbox
'''
...
class sptenmat:
    ...
    def tosptensor(self):
        ...
        return sptensor.sptensor(numpy.array(newsubs, dtype="int"), newvals, newshape);
    ...
    def tocsrmat(self):
        ...
        for i in range(0, len(self.cdims)):
            n = n * self.tsize[self.cdims[i]];
        return sparse.csr_matrix((self.vals.flatten(),
                               (self.subs[:,0], self.subs[:,1])),
            shape = [m,n]);

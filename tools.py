...
The code is the python implementation of the various functions in the MATLAB Tensor Toolbox
'''
import numpy;
import cmath;
import math;
...
def prod(arg):
    return numpy.prod(numpy.array(arg))

def accum_np(accmap, a, size, func=numpy.sum):
    """ 
    Construct a numpy array with accumulation. The numpy equivalent to the matlab function accumarray
    The elements from a data are grouped and the function is applied to each group.
    """
...
def tt_dimscheck(dims, N, M = None, exceptdims = False):
    ...
    print dims
    print numpy.array([dims])
    if(exceptdims):
        dims = listdiff(range(0,N), numpy.array([dims]))
